...
export async function dbTransaction(req: Request, res: Response, next: () => Promise<void>): Promise<void> {
  try {
...
export class RateLimiterMiddleware extends Middleware {
  ...
  static getInstance(params: Partial<RateLimiterMiddlewareParams> = {}): Middleware {
    ...
    let default_params: RateLimiterMiddlewareParams = {
      generateIdentifier: (req: Request) => req.socket.remoteAddress || 'unknown',
      maxRequests: 200,
      windowTimeSize: 30,
      windowCount: 4,
    };
    const merged_params = { ...default_params, ...params };
    return (RateLimiterMiddleware.singletonInstance = new RateLimiterMiddleware(merged_params));
  }
  ...
  async call(req: Request, res: Response, next: () => Promise<void>): Promise<void> {
    let window = parseInt((Date.now() / 1000 / this.params.windowTimeSize).toString());
    let key = `rate_limiter:${this.params.generateIdentifier(req)}:${window}`;
    let count: number = (await cache().get(key)) || 0;
    if (!count) {
      await cache().put(key, 1, this.params.windowTimeSize * (this.params.windowCount + 1));
      count = 1;
    } else {
    ...
    for (let i = 1; i < this.params.windowCount; i++) {
      count += parseInt(
        (await cache().get(`rate_limiter:${this.params.generateIdentifier(req)}:${window - i}`)) || '0'
      );
    }

    if (count > this.params.maxRequests) {
      throw new HttpTooManyRequestsError('Too many requests. Please try again later.');
    }
...
export class DatabaseProviderMiddleware extends Middleware {
  ...
  getConnection(db_config: { provider: string; config: PostgresqlConfig | MysqlConfig | SqliteConfig }): Connection {
