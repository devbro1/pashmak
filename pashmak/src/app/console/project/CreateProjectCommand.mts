...
export class CreateProjectCommand extends Command {
    ...
    this.linter = await select({
      ...
      choices: [
        ...
        },
        {
          name: 'Oxlint',
          value: 'oxlint',
          description: 'Extremely fast JavaScript/TypeScript linter and formatter (oxlint + oxfmt)',
        },
      ],
    });
  ...
  async setupGeneralPackages() {
    ...
    await this.addPackage('husky vitest supertest @types/supertest pino-pretty typescript tsx', true);
  }
  ...
  async installPackages() {
    const install_command =
      this.packageManager === 'bun' ? `bun install` : this.packageManager === 'yarn' ? `yarn` : `npm install`;
  ...
  async setupGit() {
    ...
    if (this.initGit) {
      const gitignoreContent =
        ['node_modules/', 'dist/', '.env', '.env.*', '!.env.example', '*.log', 'coverage/', '.DS_Store'].join('\n') +
        '\n';

      await fs.writeFile(path.join(this.projectPath, '.gitignore'), gitignoreContent);

      execSync(`git init; git add --all; git commit --allow-empty -m "chore: first commit"`, {
        cwd: this.projectPath,
      });
    }
  }
