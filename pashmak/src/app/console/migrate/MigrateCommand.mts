    ...
    await context_provider.run(async () => {
      ...
      if (this.refresh && (await schema.tableExists('migrations'))) {
        ...
        for (const migration_record of existing_migrations) {
          ...
          try {
            const MigrationClass = (await import(path.join(migrationsDir, migration_record.filename))).default;
            ...
            logger().error(`Failed to rollback migration ${migration_record.filename}: ${error}`);
            throw error;
          }
        }

        logger().info(`rolled back ${existing_migrations.length} migrations successfully!`);
      }
