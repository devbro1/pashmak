...
\`\`\`
    `,
    examples: [
      [`Generate from routes`, `$0 generate apidocs generate-from-routes --output path/to/output.json`],
      [`Generate base spec`, `$0 generate apidocs generate-base --output path/to/output.json`],
  ...
  async execute() {
    if (!this.subcommand) {
      this.context.stdout.write(this.constructor.usage?.toString() || 'No help available\n');
      return 0;
    }
  ...
  private async executeGenerateFromRoutes() {
    this.context.stdout.write(`Generating OpenAPI documentation from routes...\n`);
    ...
    this.context.stdout.write(`OpenAPI routes documentation generated at: ${outputPath}\n`);
    this.context.stdout.write(`Total routes documented: ${Object.keys(openApiSpec.paths).length}\n`);
    return 0;
  }
  ...
  private async executeGenerateBase() {
    ...
    this.context.stdout.write(`Base OpenAPI specification generated at: ${outputPath}\n`);
    return 0;
  }
  ...
  private async executeMergeFiles() {
    ...
    for (const file_path of files_to_merge) {
      try {
        ...
        this.context.stderr.write(`  Warning: Could not read ${file_path}: ${(error as Error).message}\n`);
      }
    }
    ...
    this.context.stdout.write(`Final OpenAPI document written to: ${outputPath}\n`);
    return 0;
  }
