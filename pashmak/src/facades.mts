...
import { Mailer, MailerProvider, MailerProviderFactory } from '@devbro/neko-mailer';
...
export const db = (label = 'default') => ctx().getOrThrow<Connection>(['database', label]);
...
export const mailer = wrapSingletonWithAccessors(
  createSingleton((label) => {
    ...
    const provider: MailerProvider = MailerProviderFactory.create(mailer_config.provider, mailer_config.config);
...
export const cache = wrapSingletonWithAccessors(
  createSingleton((label) => {
    ...
    const provider = CacheProviderFactory.create(cache_config.provider, cache_config.config);
