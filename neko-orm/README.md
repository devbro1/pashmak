...
export class Product extends BaseModel {
  ...
  // UUID primary key (DB-generated via column DEFAULT)
...
Models can use UUID primary keys instead of auto-incrementing integers. The UUID is **generated by the database** via the column's `DEFAULT` expression — no application-level UUID generation occurs.
...
// The DB generates the UUID; ORM retrieves it via RETURNING (PostgreSQL)
...
The database table must be created with a `DEFAULT` expression for UUID generation. Use `table.id({ uuid: true })` in your migration:

```ts
await schema.createTable('posts', (table) => {
  table.id({ uuid: true }); // adds DEFAULT gen_random_uuid() for PostgreSQL
  table.string('title');
  table.timestamps();
});
```
...
- `gen_random_uuid()` is built-in since PostgreSQL 13. For older versions: `CREATE EXTENSION IF NOT EXISTS "pgcrypto";`
- For UUID v7, install `pg_uuidv7`: `CREATE EXTENSION IF NOT EXISTS "pg_uuidv7";` and override the column default to `uuid_generate_v7()`

**MySQL**:
- `UUID()` is built-in (MySQL 8.0+). Generates UUID v1 (time+MAC-based).
- UUID v4/v7 are not natively supported by MySQL.
- ⚠️ MySQL's `insertGetId` relies on `LAST_INSERT_ID()`, which does not return UUID values. For MySQL UUID primary keys, use `incrementingPrimaryKey: false` and manage the UUID manually, or use a trigger.

**SQLite**:
- SQLite has no built-in UUID function. Use `incrementingPrimaryKey: false` and set the UUID before saving.
