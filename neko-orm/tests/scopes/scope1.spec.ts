...
import { type Connection, PostgresqlConnection, type Query } from '@devbro/neko-sql';
...
import type { Region } from '../fixtures/models';
...
describe('global scopes queries', () => {
  ...
  test('basic testing', async () => {
    ...
    const c1 = await (await Country.getQuery()).get();
    expect(c1.length).toBe(25);

    const c2 = await (await Country2.getQuery()).get();
    expect(c2.length).toBe(5);
    c2.forEach((c: Country2) => {
      expect(c.region_id).toBe(2);
    });

    const c3 = await (await Country3.getQuery()).get();
    ...
    const c1_1 = await (await Country.getQuery()).region(1).get();
    expect(c1_1.length).toBe(8);
    const c1_2 = await (await Country.getQuery()).region(2).get();
    expect(c1_2.length).toBe(5);
    const c1_3 = await (await Country.getQuery()).region(3).get();
    expect(c1_3.length).toBe(6);
    const c1_4 = await (await Country.getQuery()).region(4).get();
    expect(c1_4.length).toBe(6);
    const c1_5 = await (await Country.getQuery()).region(5).get();
    expect(c1_5.length).toBe(0);

    const c1_obj1 = await (await Country.getQuery()).region(1).getObject();
    expect(c1_obj1 instanceof Country).toBeTruthy();

    const c1_objs = await (await Country.getQuery()).region(1).getObjects();
    expect(Array.isArray(c1_objs)).toBeTruthy();
    for (const c of c1_objs) {
