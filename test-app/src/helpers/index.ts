...
export function diffLists<T extends BaseModel>(
  currentRelations: T[],
  newRelations: T[],
  compare_method: (item: T, list: T[]) => boolean = (item: T, list: T[]) =>
    !list.some((c: T) => c.id === item.id),
): { toAdd: T[]; toRemove: T[] } {
  const toAdd = newRelations.filter((r: T) =>
    compare_method(r, currentRelations),
  );
  const toRemove = currentRelations.filter((r: T) =>
    compare_method(r, newRelations),
  );
