...
export function ValidatedRequest(
  validationRules: yup.ObjectSchema<any> | (() => yup.ObjectSchema<any>) | z.ZodType<any> | (() => z.ZodType<any>)
): ParameterDecorator {
  return createParamDecorator(async () => {
    ...
    const rc = await (schema as yup.ObjectSchema<any>).noUnknown().validate(requestBody, { abortEarly: false });
