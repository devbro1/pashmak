export type WithoutDollarKeys<T> = T extends readonly (infer U)[]
  ? readonly WithoutDollarKeys<U>[]
...
export type DotPaths<T> = {
  [K in keyof T & string]: T[K] extends Record<string, any> ? K | `${K}.${DotPaths<T[K]>}` : K;
}[keyof T & string];
