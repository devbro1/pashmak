...
for (const loc of to_bump) {
  let pchanges = execSync(`git diff HEAD ${loc}/package.json; git diff master ${loc}/package.json`, {
    cwd: workspacePath,
    // stdio: 'inherit',
  })
    .toString()
...
for (const loc of to_bump) {
  let packageJson = JSON.parse(fs.readFileSync(path.join(loc, 'package.json'), 'utf-8'));
  let versionParts = packageJson.version.split('.').map((num) => parseInt(num, 10));
  versionParts[2] += 1; // Increment patch version
  let newVersion = versionParts.join('.');
  packageJson.version = newVersion;
  fs.writeFileSync(path.join(loc, 'package.json'), JSON.stringify(packageJson, null, 2) + '\n', 'utf-8');
