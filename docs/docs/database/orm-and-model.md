...
You can use UUID as a primary key instead of an auto-incrementing integer. When `uuid: true` is set, the UUID is **generated by the database** via a column `DEFAULT` expression. The ORM retrieves the generated UUID after insert (via `RETURNING id` on PostgreSQL).
...
// UUID is generated by the database; ORM retrieves it via RETURNING
...
The table must be created with `table.id({ uuid: true })` in the migration so the DB-level `DEFAULT` is added:
...
- For UUID v7 (time-ordered), install `pg_uuidv7`: `CREATE EXTENSION IF NOT EXISTS "pg_uuidv7";` and override the column default to `uuid_generate_v7()`

**MySQL**: `UUID()` is built-in (MySQL 8.0+, generates UUID v1). However, MySQL's `LAST_INSERT_ID()` does not return UUID values, so `incrementingPrimaryKey: true` is not supported for UUID primary keys in MySQL. Use `incrementingPrimaryKey: false` and manage the UUID manually.

**SQLite**: No built-in UUID function. Use `incrementingPrimaryKey: false` and provide the UUID before saving.
