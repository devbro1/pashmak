...
`table.id({ uuid: true })` creates a UUID-based primary key whose value is **generated by the database** at insert time. The column type and auto-generated `DEFAULT` expression differ per database:

| Database   | Column Type | Default Expression          | Notes |
|------------|-------------|-----------------------------|-------|
| PostgreSQL | `uuid`      | `DEFAULT gen_random_uuid()` | UUID v4. Built-in since PostgreSQL 13. |
| MySQL      | `CHAR(36)`  | `DEFAULT (UUID())`          | UUID v1 (time+MAC). MySQL 8.0+ required. |
| SQLite     | `TEXT`      | _(none)_                    | No built-in UUID function. |

```typescript
await schema.createTable("posts", (table) => {
  table.id({ uuid: true }); // UUID primary key with DB-level default
  table.string("title");
  table.timestamps();
});
```
...
PostgreSQL 13+ includes `gen_random_uuid()` with no extension required. For older versions or UUID v7:

```sql
-- For PostgreSQL < 13 (UUID v4 via pgcrypto)
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- For UUID v7 support (recommended for time-ordered UUIDs)
CREATE EXTENSION IF NOT EXISTS "pg_uuidv7";
```

Then override the default in your migration to use UUID v7:

```typescript
import { Expression } from "@devbro/neko-sql";

await schema.createTable("posts", (table) => {
  // Override default to use uuid_generate_v7() from pg_uuidv7 extension
  table.id({ uuid: true }).default(new Expression("uuid_generate_v7()"));
  table.string("title");
  table.timestamps();
});
```
...
No extensions needed. `UUID()` is a built-in MySQL function (MySQL 8.0+). Note that MySQL's `UUID()` generates **UUID v1** (time+MAC-address based), not UUID v4 or v7.

#### SQLite Notes

SQLite has no built-in UUID generation function. For SQLite, use `incrementingPrimaryKey: false` in the model and provide UUIDs manually, or generate them in application code.
