...
table.id({ uuid: true }); // UUID primary key (DB-generated)
...
`table.id({ uuid: true })` creates a UUID primary key whose value is **generated by the database** at insert time via a column `DEFAULT` expression. The underlying column type and default expression depend on the database:

| Database   | Column Type | Default Expression       | Notes                                     |
|------------|-------------|--------------------------|-------------------------------------------|
| PostgreSQL | `uuid`      | `DEFAULT gen_random_uuid()` | UUID v4. Built-in since PostgreSQL 13. For older versions enable `pgcrypto`: `CREATE EXTENSION IF NOT EXISTS "pgcrypto";`. For UUID v7 install `pg_uuidv7`: `CREATE EXTENSION IF NOT EXISTS "pg_uuidv7";` then override the default to `uuid_generate_v7()`. |
| MySQL      | `CHAR(36)`  | `DEFAULT (UUID())`       | UUID v1. Requires MySQL 8.0+. `UUID()` generates a v1 (time-based) UUID; MySQL does not natively support UUID v4/v7. |
| SQLite     | `TEXT`      | _(none)_                 | SQLite has no built-in UUID function. UUID generation must be handled at the application level. |

#### PostgreSQL Setup

```sql
-- For PostgreSQL < 13: enable pgcrypto for gen_random_uuid()
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- For UUID v7 support, install the pg_uuidv7 extension:
CREATE EXTENSION IF NOT EXISTS "pg_uuidv7";
```

Then override the default in migrations:

```typescript
// UUID v4 (default, no extension needed on PG 13+)
await schema.createTable('posts', (table) => {
  table.id({ uuid: true }); // DEFAULT gen_random_uuid()
  table.string('title');
  table.timestamps();
});

// UUID v7 (requires pg_uuidv7 extension)
await schema.createTable('posts', (table) => {
  const idCol = table.id({ uuid: true });
  idCol.default(new Expression('uuid_generate_v7()')); // override default
  table.string('title');
  table.timestamps();
});
```
...
```sql
-- No extensions needed. UUID() is a built-in function in MySQL 8.0+.
-- Note: MySQL's UUID() generates UUID v1 (time+MAC-based), not v4 or v7.
```

```typescript
await schema.createTable('users', (table) => {
  table.id({ uuid: true }); // CHAR(36) NOT NULL DEFAULT (UUID())
  table.string('name');
});
```
...
`table.uuid('column')` creates a bare UUID-typed column with no auto-default. You control when and how the value is set:

```typescript
await schema.createTable('events', (table) => {
  table.id();
  table.uuid('correlation_id'); // no default — set value explicitly
  table.string('name');
});
```
